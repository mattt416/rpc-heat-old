...
if [ $(ssh -l root -i $ssh_key $ssh_options $ip "test -d ${checkout}/playbooks"; echo $?) -eq 0 ]; then
  playbooks="${checkout}/playbooks"
else
  playbooks="${checkout}/rpc_deployment/playbooks"
fi
...
ssh -l root -i $ssh_key $ssh_options $ip "sed -i 's/api_workers = 10/api_workers = 0/' ${playbooks}/roles/neutron_common/templates/neutron.conf"
ssh -l root -i $ssh_key $ssh_options $ip "sed -i 's/rpc_workers = 5/rpc_workers = 0/' ${playbooks}/roles/neutron_common/templates/neutron.conf"
